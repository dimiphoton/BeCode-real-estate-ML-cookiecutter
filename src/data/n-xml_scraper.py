...
def run(n):
    columns_names=["Locality","House or appartment?", "Subtype","Price", "Type of sale", "Number of rooms",
    "Living area","Fully equipped kitchen","Furnished", "Open fire", "Terrace","Area of terrace",
    "Garden", "Area of garden", "Land surface","Number of facades", "Swimming pool", "State of building"]
    ...
    properties_split = np.array_split(propertieslist[0:40], os.cpu_count())
...
# Copying the data collected into a .csv file
    df.to_csv('Immoweb_data'+'{0:03}'.format(n)+'.csv', index=False, header=True)
